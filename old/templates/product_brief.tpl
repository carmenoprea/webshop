...
{if $product_info[7]}
	<a href="index.php?productID={$product_info[11]}">
	 <img src="products_pictures/{$product_info[7]}" alt="{$product_info[1]}" border=0><br>
	 {$smarty.const.MORE_INFO_ON_PRODUCT}
	</a>
{else}
  {if $product_info[5]}
	<a href="index.php?productID={$product_info[11]}">
	 <img src="products_pictures/{$product_info[5]}" alt="{$product_info[1]}" border=0>
	 {$smarty.const.MORE_INFO_ON_PRODUCT}
	</a>
  {/if}
{/if}
...
<td valign=top width=99%>
 ...
 <td align=right valign=top> {* arr to cart *}

	{if $smarty.const.CONF_SHOW_ADD2CART eq 1}
	 {if $product_info[6] > 0}
			<a href="index.php?shopping_cart=yes&add2cart={$product_info[11]}"><img src="images/cart_small.gif" border=0 alt="{$smarty.const.ADD_TO_CART_STRING}"></a>
	 {else}&nbsp;
	 {/if}
	{/if}
 ...
 {if $currencies_count ne 0}
	...
	<b>{$smarty.const.CURRENT_PRICE}: <font class=cat color=red>
